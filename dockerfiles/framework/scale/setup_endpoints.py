...
def designate_endpoints(tar_zip_url, certs_endpoints_url):
    if tar_zip_url and certs_endpoints_url:  # check existance of CERTS_ENDPOINTS_URL and CERTS_TAR_ZIP
        ...
        if tar_zip_split[-1] == "zip":  # zip file
            ...
            # TODO check existance of CERTS_ENDPOINTS_URL and CERTS_TAR_ZIP
            # if exists, determine if zip or tar
            # place certs in /etc/pki/ca-trust/source/anchors
            # place endpoints.json in /usr/lib/python2.7/site-packages/botocore/data/endpoints.json
